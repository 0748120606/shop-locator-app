...
if (!empty($date_from) && !empty($date_to)) {
    $where .= " AND DATE(r.date_posted) BETWEEN '$date_from' AND '$date_to'";
}

$query = "SELECT r.rating_id, r.rating, r.review, r.date_posted,
...
<head>
  <meta charset="UTF-8">
  <title>Ratings & Reviews Report</title>
  <style>
...
<body>
      ...
      <?php if ($result && $result->num_rows > 0): ?>
        <?php while ($row = $result->fetch_assoc()): ?>
          <tr>
            ...
            <td><?= htmlspecialchars($row['review']) ?: 'No comment' ?></td>
            <td><?= date('M d, Y', strtotime($row['date_posted'])) ?></td>
          </tr>
        <?php endwhile; ?>
      <?php else: ?>
        <tr><td colspan="5">No ratings or reviews available.</td></tr>
      <?php endif; ?>
    </tbody>
  </table>
  ...
  <script>
    function exportTableToExcel(tableID, filename = 'ratings_reviews_report.xls') {
